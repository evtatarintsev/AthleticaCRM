...
export function LoginPage({
  ...
  const onCredentials = async (event: FormEvent<HTMLFormElement>) => {
    ...
    return (
      <AuthLayout title={t.branchTitle} subtitle={t.branchSubtitle}>
        <div className="space-y-3">
          ...
          <button
            type="button"
            onClick={() => setState({ step: "credentials", loading: false, error: null })}
  ...
  return (
    ...
    >
      <form method="post" onSubmit={onCredentials} className="space-y-4">
        {state.error && <ErrorAlert message={state.error} />}
        <TextField
          label={t.email}
          name="username"
