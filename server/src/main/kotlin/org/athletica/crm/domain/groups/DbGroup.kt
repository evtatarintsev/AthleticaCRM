...
class DbGroup(
    ...
    context(ctx: RequestContext, tr: Transaction, raise: Raise<DomainError>)
    override suspend fun withNewSchedule(schedule: List<ScheduleSlot>): Group = DbGroup(id, name, schedule, disciplines)

    context(ctx: RequestContext, tr: Transaction, raise: Raise<DomainError>)
    override suspend fun withNewDisciplines(disciplines: List<DisciplineId>): Group = DbGroup(id, name, schedule, disciplines)

    context(ctx: RequestContext, tr: Transaction, raise: Raise<DomainError>)
    override suspend fun withNewName(name: String): Group = DbGroup(id, name, schedule, disciplines)
}
